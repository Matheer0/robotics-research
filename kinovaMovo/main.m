...
q       = joint_pos_init ;
q_param = [q(1:dim_joint). JointPosition]';   % 29x1 matrix  
dq      = zeros(size(q_param)) ;              % 29x1 matrix 
state   = [q_param', dq'] ;                   % 1x58 matrix
...
q_opt_param = [q_opt(1:dim_joint). JointPosition]';   % 29x1 matrix   
dq_opt      = zeros(size(q_opt_param)) ;              % 29x1 matrix 
state_opt   = [q_opt_param', dq_opt'] ;               % 1x58 matrix
